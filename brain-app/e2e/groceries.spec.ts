...
test('manages lists, items, completion and sharing without writing real data',async({page})=>{
 ...
 await page.goto('/groceries');await expect(page.getByRole('heading',{name:'Einkaufsliste'})).toBeVisible();
 ...
 await page.getByLabel('Eintrag für Party').fill('Chips');await page.getByLabel('Eintrag für Party').press('Enter');await expect(page.getByText('Chips')).toBeVisible();
