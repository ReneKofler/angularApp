...
test('loads body progress and opens each editor',async({page})=>{
  ...
  await page.getByRole('button',{name:'Ziele',exact:true}).click();await expect(page.getByRole('heading',{name:'Ziele ändern'})).toBeVisible();await page.getByRole('button',{name:'Abbrechen'}).click();
